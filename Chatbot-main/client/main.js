...
let userQustionCount = 0;
let lastQuestionDate = null;

// adding check function to check daily question limit
const checkQuestionLimit = () => {
    const today = new Date().toDateString();

    // check if it is new day reset the counter
    if(lastQuestionDate !== today) {
        userQustionCount = 0;
        lastQuestionDate = today;
    }

    // check if user exceeded the daily limit
    return userQustionCount < MAX_QUESTIONS_PER_DAY;
};
